...
public class NNTrainPatterns : NNForwardPropagation
{
    ...
    public uint Misrecognitions;
    public bool NeedHessian;
    public int Backprops;
                        ...
                        MainForm form, List<Mutex> mutexs)
    {
        ...
        IsDataReady = trainingDataReady;
        Network = neuronNet;
        MnistDataSet = trainingSet;
        Preferences = preferences;
        ImageCount = (uint)MnistDataSet.ImagePatterns.Count;
        MainForm = form;
        StopEvent = eventStop;
    ...
    {
        AfterEveryNBackprops = nAfterEveryNBackprops;
        EtaDecay = dEtaDecay;
        MinimumEta = dMinimumEta;
        EstimatedCurrentMSE = dEstimatedCurrentMSE;  // this number will be changed by one thread and used by others
        ShouldDistortPatterns = bDistortTrainingPatterns;
    }
    /// <summary>
    /// 
    /// </summary>
    ...
    void CalculateHessian()
    {
        ...
        MainForm?.Invoke(MainForm.DelegateAddText, [3, s]);

        // some of this code is similar to the BackpropagationThread() code

        Network.EraseHessianInformation();

        uint numPatternsSampled = Preferences.NumHessianPatterns;

        for (kk = 0; kk < numPatternsSampled; ++kk)
        {
            int iRandomPatternNum;

            iRandomPatternNum = MnistDataSet.GetRandomPatternNumber();
            byte label = MnistDataSet.ImagePatterns[iRandomPatternNum].Label;

            if (label < 0) label = 0;
            if (label > 9) label = 9;
            var grayLevels = MnistDataSet.ImagePatterns[iRandomPatternNum].Pattern;
            ...
            Network.Calculate(inputVector, 841, actualOutputVector, 10, null);


            // backpropagate the second derivatives

            Network.BackpropagateSecondDervatives(actualOutputVector, targetOutputVector, 10);
            ...
            if (StopEvent.WaitOne(0, true))
            {
                ...
                // ...
                var ss = "BackPropagation stoped";
                // Make synchronous call to main form.
                // MainForm.AddString function runs in main thread.
                // To make asynchronous call use BeginInvoke
                MainForm?.Invoke(MainForm.DelegateAddText, [3, ss]);
                                   ...
                                   bool bDistort)
    {
        ...
        Mutexs[2].WaitOne();
        if (AfterEveryNBackprops == 0) AfterEveryNBackprops = 1;
        if (((Backprops % AfterEveryNBackprops) == 0) && (Backprops != 0))
        {
            var eta = Network.EtaLearningRate;
            eta *= EtaDecay;
            if (eta < MinimumEta)
                eta = MinimumEta;
            Network.EtaLearningRatePrevious = Network.EtaLearningRate;
            Network.EtaLearningRate = eta;
        }
        ...
        if ((Backprops % Preferences.ItemsTrainingImages) == 0)
        {
        ...
        bWorthwhileToBackpropagate = dMSE > (0.10 * EstimatedCurrentMSE);
        ...
        if ((bWorthwhileToBackpropagate) && (pMemorizedNeuronOutputs == null))
        {
            ...
            Network.Backpropagate(actualOutputVector, targetOutputVector, oCount, null);
    ...
    public void BackpropagationThread()
    {
        ...
        byte label = 0;
        int ii, jj;
        NNNeuronOutputsList _memorizedNeuronOutputs = [];
        ...
        while (true)
        {
            ...
            BackpropagateNeuralNet(inputVector, 841, targetOutputVector, actualOutputVector, 10,
                _memorizedNeuronOutputs, ShouldDistortPatterns);
            ...
            {
                MSE200 /= 200;
                s = "MSE:" + MSE200.ToString();
                MainForm.Invoke(MainForm.DelegateAddText, [4, s]);
                MSE200 = 0;
                NN = 0;
            }
            ...
            // MainForm.AddString function runs in main thread.
            // To make asynchronous call use BeginInvoke
            MainForm?.Invoke(MainForm.DelegateAddText, new Object[] { 5, s });

            if (NextPattern >= MnistDataSet.ImagePatterns.Count - 1)
            {
                Timer.Stop();
                MSE /= NextPattern;
                s = string.Format("Completed Epochs:{0}, MisPatterns:{1}, MSE:{2}, Ex. time: {3}, eta:{4} ",
                    ...
                    (Misrecognitions),
                    MSE, 
                    Timer.Duration, Network.EtaLearningRate.ToString());
                // Make synchronous call to main form.
                // MainForm.AddString function runs in main thread.
                // To make asynchronous call use BeginInvoke
                MainForm?.Invoke(MainForm.DelegateAddText, new Object[] { 3, s });
                Misrecognitions = 0;
                EpochsCompleted++;
                NextPattern = 0;
                MSE = 0;
